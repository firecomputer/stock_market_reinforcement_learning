...
class MarketPolicyGradientModelBuilder(AbstractModelBuilder):
	...
	def buildModel(self):
                ...
                for i in range(1):
                        ...
                        h = Convolution2D(2048, 3, 1, padding='same')(S)
                        h = LeakyReLU(0.001)(h)
                        h = Convolution2D(2048, 5, 1, padding="same")(S)
                        h = LeakyReLU(0.001)(h)
                        h = Convolution2D(2048, 10, 1, padding='same')(S)
                        h = LeakyReLU(0.001)(h)
                        h = Convolution2D(2048, 20, 1, padding='same')(S)
                        h = LeakyReLU(0.001)(h)
                        h = Convolution2D(2048, 40, 1, padding='same')(S)
                        h = LeakyReLU(0.001)(h)
...
class MarketModelBuilder(AbstractModelBuilder):
	...
	def buildModel(self):
		...
		m = keras.layers.merge(merges, mode = 'concat', concat_axis = 1)
		m = Dense(1024)(m)
		m = LeakyReLU(0.001)(m)
		m = Dropout(dr_rate)(m)
		m = Dense(512)(m)
